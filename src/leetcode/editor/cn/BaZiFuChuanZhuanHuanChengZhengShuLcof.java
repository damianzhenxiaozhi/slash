...
public class BaZiFuChuanZhuanHuanChengZhengShuLcof {
    public static void main(String[] args) {
        Solution s = new BaZiFuChuanZhuanHuanChengZhengShuLcof().new Solution();
    ...
    class Solution {
        public int strToInt(String str) {
            ...
            if (isDigital(input[i])) {
                return strToInt(input, i, N - 1, '+');
            } else if (isSign(input[i])) {
                return strToInt(input, i + 1, N - 1, input[i]);
            } else {
                return 0;
            }
        }
        ...
        private int strToInt(char[] arr, int i, int j, char sign) {
            if (i > j) {
                return 0;
            }
            ...
            if (sign == '-') {
                result = -result;
                while (i <= j && isDigital(arr[i])) {
                    if ((Integer.MIN_VALUE + (arr[i] - '0')) / 10 > result) {
                ...
                while (i <= j && isDigital(arr[i])) {
                    if ((Integer.MAX_VALUE - (arr[i] - '0')) / 10 < result) {
